...
function Binder.new(tagName, constructor)
	...
	task.delay(5, function()
		if not self._loaded then
			warn("Binder is not loaded. Call :Run() on it!")
		end
	end)
...
function Binder:Run()
	...
	for _, inst in pairs(CollectionService:GetTagged(self._tagName)) do
		task.spawn(function()
			local Success, Error = pcall(self._add, self, inst)
			if not Success then
				warn(string.format("[Binder] Failed to add instance for tag '%s': %s", self._tagName, Error))
			end
		end)
	end
end
...
function Binder:Bind(inst)
	if RunService:IsClient() then
		warn(
			("[Binder.Bind] - Bindings '%s' done on the client! Will be disrupted upon server replication!"):format(
				self._tagName
			)
		)
	end
...
function Binder:Unbind(inst)
	...
	if RunService:IsClient() then
		warn(
			("[Binder.Bind] - Unbinding '%s' done on the client! Might be disrupted upon server replication!"):format(
				self._tagName
			)
		)
	end
