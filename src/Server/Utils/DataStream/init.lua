...
--[[
    HANDLE_PLAYER_REMOVAL (Custom Code)

    Set to false if you will handle the DataStream removal manually.
    This is useful when you want to get the DataStream object before the player leaves, to then save it.

    Set true to handle automatically.
    Good for not-data-connected projects.

--]]
local HANDLE_PLAYER_ADDED = true
local HANDLE_PLAYER_REMOVED = false
...
function DataStream:MakePlayerStream(name: string, schema: { [any]: any })
...
-- Handle Player Added
function DataStream:HandlePlayerAdded(player: Player)
	for name, _ in pairs(Replicating.Player) do
		local cachedSchema = SchemaCache[name]
		self:MakeStreamForPlayer(name, player, DataStreamUtils:DeepCopyTable(cachedSchema))
	end
end

-- Handle Player Removed
function DataStream:HandlePlayerRemoved(player: Player)
...
do
	...
	--// Handle Player Added
	if HANDLE_PLAYER_ADDED then
		PlayerAddedHelper:OnPlayerAdded(function(player)
			DataStream:HandlePlayerAdded(player)
		end)
	else
		print("DataStream:Init() - Player Added is now being handled manually.")
	end

	if HANDLE_PLAYER_REMOVED then
		Players.PlayerRemoving:Connect(function(player)
			DataStream:HandlePlayerRemoved(player)
		end)
	else
		print("DataStream:Init() - Player Removed is now being handled manually.")
	end

	--// Handle GetData Function
