...
local MODULE_PATHS = {
	...
	RunService:IsClient()
			and {
				Alias = "Client",
				Instance = (if RunService:IsRunning()
					then Players.LocalPlayer:WaitForChild("PlayerScripts")
					else StarterPlayer:WaitForChild("StarterPlayerScripts")):WaitForChild("ClientModules"),
			}
		or {
			Alias = "Server",
	...
	{
		Alias = "Configs",
		Instance = ReplicatedStorage:WaitForChild("Configs"),
	},
}
...
local function FindModule(query: string | ModuleScript, _currentTimeout: number?): ModuleData?
	...
	elseif #found > 1 and type(query) == "string" then
		warn(
			"Multiple modules found with the name '"
				.. query
				.. "'. To clarify, please use the path instead. "
				.. string.gsub("(ex: Shared/Framework/Module)", "/", CONFIG.PATH_SEPERATOR)
		)
	end
...
local function NewRequire(query: string | ModuleScript, _fromInternal: boolean?): any
	...
	if not targetModuleData then
		...
		local function onRequestedError()
			...
			warn(
				"Module "
					.. (requirerModuleData and requirerModuleData.Path or requirer.Name)
					.. " cannot continue until "
					.. targetModuleData.Path
					.. " is fixed"
			)
				...
				if tick() - timeStart > CONFIG.LONG_LOAD_TIMEOUT then
					...
					if _fromInternal ~= true then
						...
						if requirer then
							...
							if cycleString then
								warn(
									"Cyclical require detected: (" .. cycleString .. ").\nPlease resolve this issue at",
									string.gsub(debug.traceback(currentThread, "", 3), "\n", "")
								)
								return
							end
						end
...
local function DoFirstRequire(moduleData: ModuleData)
	...
	if not success then
		warn(
			"Module",
			moduleData.Path,
			"failed to auto-load. Check output for error from:",
			moduleData.Instance:GetFullName()
		)
	end
			...
			if type(moduleQuery) ~= "userdata" or moduleQuery:IsA("ModuleScript") then
				task.spawn(function()
					...
					if moduleData then
						...
						warn(
							"Failed to add module to init flow:",
							moduleQuery,
							"not found in Eden directory. Please verify the spelling and/or path."
						)
					end
				...
				warn(
					"Failed to add module to init flow: Instance",
					moduleQuery:GetFullName(),
					"is not a valid module query."
				)
				addedCount += 1
			end
		end
		...
		local function tryFinalize()
			...
			for index, moduleQuery in ipairs(initFirstArray :: { any }) do
				...
				if moduleData then
					...
					warn(
						"Failed to prioritize module from initFirst table:",
						moduleQuery,
						"not found. Please verify the spelling and/or path."
					)
				end
			end
			...
			statusConnection = RunService.Heartbeat:Connect(function(deltaTime)
				...
				if status == "dead" and InitalizedModules == false then
					...
					warn(
						"Module",
						focusedModuleData.Path,
						"failed to :Init(). Error must be resolved or modules next in priority will not execute :Init()"
					)
					return
				end
			...
			for _, moduleData in ipairs(Modules) do
				if moduleData.AutoInitData.Init then
					...
					initTime = 0
					if
						moduleData.AutoInitData.HasExplicitPriority == true
						or CONFIG.PCALL_NON_PRIORITY_MODULES == false
					then
						moduleData.AutoInitData.Init(moduleData.AutoInitData.RequiredData)
					else
						local success, initError =
							pcall(moduleData.AutoInitData.Init, moduleData.AutoInitData.RequiredData)
						if not success then
		...
		if moduleInstance:IsA("ModuleScript") then
			...
			if success and InitModulesPhaseInt > 0 or InitalizedModules == true then
				warn(
					...
					"replicated late to",
					pathData.Alias,
					"module folder. This may cause unexpected behavior."
				)
			end
		end
	end)
